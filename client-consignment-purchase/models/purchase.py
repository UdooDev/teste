...
class purchase_order(models.Model):
    ...
    def button_confirm(self):
        ...
        print("=======con_purchase======",self.order_type)
        if self.order_type == 'con_purchase':
            if len(self.order_line) < 1:
                ...
                # estoque = self.env['stock.quant'].sudo().search([('location_id.id', '=', self.partner_id.consignee_location_id.id)])
                ...
                for item in self.order_line:
                    ...
                    #             flag = True
                    stock_quant.sudo().create({'product_id': item.product_id.id, 'quantity': item.product_qty, 'location_id': self.partner_id.consignee_location_id.id})
                if bool(excluir): 
