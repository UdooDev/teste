...
class res_partner(models.Model):
    ...
    def create_xls_poconsignment_report(self):
        estoque = self.env['stock.quant'].search([('consig_op_type', '=', 'purchase'),('location_id.id', '=', self.consignee_location_id.id)])
        if len(estoque) > 0:            
    ...
    def default_get(self, fields):
        ...
        if result.get('template_id') and result.get('template_id') == consignment_template.id:
            if self._context and self._context.get('active_id'):
                attachment_id = self.env['res.partner'].browse(self._context.get('active_id')).create_xls_poconsignment_report()
